...
export const SearchClassification: React.FC<SearchClassProps> = ({
  showItems,
  setShowItems,
  classificationLevel,
  classificationArray,
  ...
  useEffect(() => {
    ...
    const fetchTaxon = () => {
      setLoading(true);
      axios
        .get(`${baseUrlApi}/species/${classificationArray[classificationLevel - 1].value}/children`, {
          params: {
            limit: 20,
            offset: offset,
          },
        ...
        .then((response) => {
          ...
          if (
            ...
            tempArray[tempArray.length - 1].rank === classificationArray[classificationLevel].name.toUpperCase()
          ) {
            offset += 20;
            fetchTaxon();
          } else {
  ...
  if (filteredTaxon.length <= 0) {
    return (
      <div className={styles.noTaxonFoundContainer}>
        <p className={`${styles.noTaxonFound} ${darkMode ? styles.lightText : undefined}`}>{`No ${classificationArray[
          classificationLevel
