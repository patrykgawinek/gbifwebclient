		...
		<Container className='d-flex flex-column'>
			{loading ? (
				...
				<>
					<Row className='mb-2'>
						<Col>
							<h2 className={darkMode ? styles.lightText : undefined}>Found {foundResults?.count.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} results</h2>
						</Col>
						<Col className='d-flex justify-content-end align-items-center'>
							<Button onClick={handleOnClick} className={`${styles.heatmapButton} ${lastSelection === -1 ? "disabled" : ""} ${darkMode ? "btn-dark" : "btn-primary"}`}>
					...
					<Row xs={12} className='align-self-center justify-content-center'>
						<Col xs={2} className='d-flex justify-content-end'>
							<Button
								className={`${styles.buttonHeight} ${offset === 0 ? `disabled` : undefined} ${darkMode ? "btn-dark" : "btn-primary"}`}
								onClick={() => {
									if (offset > 0) {
										setOffset(offset - 12);
									}
						...
						<Col xs='auto' md='auto' className={`d-flex justify-content-center align-items-center ${styles.offsetText} ${darkMode ? styles.lightText : undefined}`}>
							{foundResults?.count > 0 ? offset + 1 : 0} - {foundResults?.endOfRecords ? foundResults?.count : offset + 12}
						</Col>
						<Col xs={2}>
							<Button
								className={`${styles.buttonHeight} ${foundResults?.endOfRecords !== undefined && foundResults.endOfRecords ? `disabled` : undefined} ${darkMode ? "btn-dark" : "btn-primary"}`}
								onClick={() => {
